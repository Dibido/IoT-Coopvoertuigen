...
void Vehicle::drive()
{
  setFrontDistance();
  Serial.println("PATH:");
  Serial.println(drivingCorrection.getLeftInitDist());
  Serial.println(drivingCorrection.getRightInitDist());
  //If merging
  if (merging)
  {
...
void Vehicle::handleMessage(const Xtea& x, const String& message, const char& messageCode)
{
  ...
  switch ((char)decrypted[CHAR_POSITION]) // The childSensorId is used to indicate the message type
  {
    ...
    case START_MERGING:
      ...
      Serial.println("herrow");
      Serial.println(uint16_t(decrypted[1]));
      Serial.println(uint16_t(decrypted[2]));
      break;
