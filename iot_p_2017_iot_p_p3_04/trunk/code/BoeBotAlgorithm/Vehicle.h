...
class Vehicle
{
  public:
    ...
    */
    void handleMessage(const Xtea& x, const String& message, const char& messageCode);

    /**
      @brief Let the vehicle start driving
    */
    ...
    */
    void startMerging(const uint16_t leftDist, const uint16_t rightDist);
    /**
        @brief Switches the exiting boolean to true or false
    */
    void startExiting(bool exiting);
    /**
       @brief Switches the stopping boolean to true or false
    */
    void stopDriving(bool stopping);

    /**
        @brief: Starts merging the vehicle into the row after checking for passing vehicles
        @param: Message to start merging, contains the new pathvalues
    */
    void prepareMerge(const uint16_t leftDist, const uint16_t rightDist);
    /**
    ...
    */
    void handleSpeed();
    /**
        @brief Let's the vehicle exit his current lane to the side of the road. The vehicle also stops at the end of this function
    */
    void exitLane();
    // Getters
  ...
  private:
    ...
    uint32_t previousExitTime;
    uint32_t previousRightLedTime;
    /*
